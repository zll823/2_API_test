...
def open_apis(request, id):
    ...
    return render(request, 'welcome.html',
                  {"whichHTML": "P_apis.html", "oid": project_id, **glodict(request), "ooid": page})
...
def look_concurrent_report(request, pid):
    ...
    # 设计总res
    res = {"result": True, "cases": [], "cases_length": len(cases), "cases_wrong_length": 0,
           "cases_right_length": len(cases)}
    for case in cases:
        tmp = {"case_id": case.id, "case_name": case.name, "result_case": True, "steps": []}
        # 循环填充
        steps = DB_step.objects.filter(Case_id=case.id)
        for step in steps:
            tmp_step = {"step_id": step.id, "step_name": step.name, "result_step": "", "data": {}}
            ...
            if s_report.assert_result in [None, '', ' ']:
                ...
                for i in json.loads(s_report.assert_result).keys():
                    if not json.loads(s_report.assert_result)[i]:
                        tmp_step["result_step"] = False
                        break
                else:
                    tmp_step["result_step"] = True
            tmp["steps"].append(tmp_step)
            if not tmp_step["result_step"]:
                tmp["result_case"] = False

        if not tmp["result_case"]:
